...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(messages != null && messages.length > 0) {
            ...
            //보낸 날짜와 시간
            Date receicedDate = new Date(messages[0].getTimestampMillis());
            Log.d(TAG, " receicedDate : " + receicedDate);
            
            //메세지 내용
            String contents = messages[0].getMessageBody();
            Log.d(TAG, "contents : " + contents);

            //인텐느를 만들어서 데이터를 넣은후 액티비티를 띄운다.
            Intent disIntent = new Intent(context, SmsDisActivity.class);

            //액티비티가 아닌곳에서 인텐트를 만들어 사용할떄
            // 2개다 추가해야한다.
            // 싱글 탑은 똑같은 화면이 띄워져 있는 상태라면
           disIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP);

            disIntent.putExtra("sender", sender);
            disIntent.putExtra("receicedDate", receicedDate);
